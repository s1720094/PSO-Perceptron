import numpy as np
import itertools
import Particle from Particle.py
...
class PSO: # all the material that is relavant at swarm leveel
    ...
    def __init__(self, w, a1, a2, dim, population_size, time_steps, search_range, points):
        ...
        for label in points:
            temp_swarm = [Particle(dim,-search_range,search_range, points[label], label, points) for i in range(population_size/2)]
            self.swarm.append(temp_swarm)
